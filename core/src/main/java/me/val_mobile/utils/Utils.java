...
public class Utils {
    ...
    public static String toLowercaseAttributeName(@Nonnull Attribute atr) {
        if (atr == Attribute.ATTACK_DAMAGE) return "generic.attack_damage";
        if (atr == Attribute.ATTACK_SPEED) return "generic.attack_speed";
        if (atr == Attribute.ATTACK_KNOCKBACK) return "generic.attack_knockback";
        if (atr == Attribute.ARMOR) return "generic.armor";
        if (atr == Attribute.ARMOR_TOUGHNESS) return "generic.armor_toughness";
        if (atr == Attribute.FLYING_SPEED) return "generic.flying_speed";
        if (atr == Attribute.FOLLOW_RANGE) return "generic.follow_range";
        if (atr == Attribute.KNOCKBACK_RESISTANCE) return "generic.knockback_resistance";
        if (atr == Attribute.MAX_HEALTH) return "generic.max_health";
        if (atr == Attribute.MOVEMENT_SPEED) return "generic.movement_speed";
        if (atr == Attribute.HORSE_JUMP_STRENGTH) return "horse.jump_strength";
        if (atr == Attribute.ZOMBIE_SPAWN_REINFORCEMENTS) return "zombie.spawn_reinforcements";
                ...
                if (meta.hasAttributeModifiers() && meta.hasLore()) {
                    ...
                    if (!(modifiers == null || modifiers.isEmpty() || lore == null)) {
                        ...
                        for (AttributeModifier modifier : modifiers) {
                            if (modifier.getSlotGroup() == EquipmentSlotGroup.MAINHAND || modifier.getSlotGroup() == EquipmentSlotGroup.ANY) {
    ...
    public static EquipmentSlot getCorrectEquipmentSlot(@Nonnull Attribute attribute, @Nonnull Material material) {
        ...
        if (attribute == Attribute.ARMOR || attribute == Attribute.ARMOR_TOUGHNESS) {
            if (isHelmet(material)) return EquipmentSlot.HEAD;
            if (isChestplate(material)) return EquipmentSlot.CHEST;
            if (isLeggings(material)) return EquipmentSlot.LEGS;
            if (isBoots(material)) return EquipmentSlot.FEET;
        }
        return null;
    }
    ...
    public static Attribute translateInformalAttributeName(@Nonnull String name) {
        switch (name) {
            ...
            case "Toughness": return Attribute.ARMOR_TOUGHNESS;
            case "FlyingSpeed": return Attribute.FLYING_SPEED;
            case "FollowRange": return Attribute.FOLLOW_RANGE;
            ...
            case "HorseJumpStrength": return Attribute.HORSE_JUMP_STRENGTH;
            case "ZombieSpawnReinforcements": return Attribute.ZOMBIE_SPAWN_REINFORCEMENTS;
            default: return org.bukkit.Registry.ATTRIBUTE.get(org.bukkit.NamespacedKey.minecraft(name.toLowerCase()));
        }
    }
