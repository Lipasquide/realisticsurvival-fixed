...
public class RSVItem extends ItemStack {
        ...
        if (attributes != null) {
            ...
            if (atrConfig.getConfigurationSection(attributesPath) != null) {
                ...
                for (String s : keys) {
                    ...
                    if (atrName != null) {
                        EquipmentSlotGroup slotGroup = slot != null ? EquipmentSlotGroup.getByName(slot.name()) != null ? EquipmentSlotGroup.getByName(slot.name()) : mapSlotToGroup(slot) : EquipmentSlotGroup.ANY;
                        NamespacedKey modKey = new NamespacedKey(RSVPlugin.getPlugin(), atrName.replace(".", "_") + "_" + UUID.randomUUID());
                        AttributeModifier atrMod = new AttributeModifier(modKey, correctValue, AttributeModifier.Operation.ADD_NUMBER, slotGroup);
    ...
    private static EquipmentSlotGroup mapSlotToGroup(@Nonnull EquipmentSlot slot) {
        return switch (slot) {
